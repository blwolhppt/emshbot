...
admin_id = -1001814371136
support_chat = -1001814371136
...
def send_to_staff(message):
    ...
    issues.message, users.phone_number from issues left join users on users.telegram_id = issues.person_id order by issues.id desc limit 1''')[
            0]
        person = None
        if (te[5] != None):
            person = te[5]
...
def callback_inline(call):
    ...
    if (call.data == 'solved'):
        ...
        make_query(
            "update issues set solved = 1, time_solved = ?, whosolved_tel_id = ?, solved_text = 'by_button' where id = ?",
...
def ask_geophone(message):
    try:
        ...
        msg = bot.send_message(message.chat.id,
                               "Пожалуйста, отправьте нам Ваш номер телефона, чтобы мы могли идентифицировать Вас",
...
def echo(message):
    if (message.chat.type == 'private' and check_user(message)):
        ...
        try:
            ...
            make_query(
                '''update issues set solved = 1, whosolved_tel_id = ?, time_solved = ?, solved_text = ? where id = ?''',
